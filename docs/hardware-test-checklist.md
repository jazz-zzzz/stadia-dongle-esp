...
5. 将 Assistant 短按动作临时改为 `F15`，点击 **保存映射**。
...
3. 用键盘 Tab 将焦点停在某个 **忘记此手柄** 按钮或映射控件上并等待 10 秒；确认实时轮询不会夺走焦点或替换正在操作的控件。
4. 连续执行 **启动 Wi-Fi 热点** / **停止 Wi-Fi 热点** 至少 5 轮；确认页面状态一致、USB 配置始终可用，固件无重启或死锁。
...
7. 从 USB 页面点击 **重启接收器**；确认先显示预期重启状态，设备断开时不出现矛盾的失败提示，重枚举后可以重新连接。
